...
## 現況
v1.4 / versionCode 6。Kotlin、UIは全てコード生成（XMLレイアウトなし）。
...
1. **日報の自動送信** — 現在は通知＋手動共有。Gmail連携やIFTTT webhookで自動送付まで繋げられる
2. **週報・月報** — `Report.build()` の期間指定を広げるだけで作れる（dayOffset を from/to に一般化）
3. **予兆の抑制設定** — 端末ごとに「この指標は無視」を持たせる。誤検知が続く端末が出てきたら必要になる
